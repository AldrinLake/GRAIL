...
class GRAILNet(IncrementalNet):
    ...
    def update_fc_aug(self, num_old, num_total, num_aux):
        fc = self.generate_fc(self.feature_dim, num_total+num_aux)
        if self.fc_aug is not None:
            weight = copy.deepcopy(self.fc_aug.weight.data)
            bias = copy.deepcopy(self.fc_aug.bias.data)
    ...
    def forward(self, x):
        out = self.convnet(x)             #return {'fmaps': [x_1, x_2, x_3, x_4], 'features': features}
        return out
