...
class BaseLearner(object):
    ...
    def _evaluate(self, y_pred, y_true):
        ret = {}
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes, init_class=self.init_cls, increment= self.increment)
        ret["grouped"] = grouped
    ...
    def _eval_nme(self, loader, class_means):
        class_means = torch.stack(class_means).cpu().numpy()
        # print(class_means.shape)
        self._network.eval()
        vectors, y_true = self._extract_vectors(loader)
        # vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T  # sample-wise normalization
    ...
    def _eval_ncm(self, loader, class_means):
        ...
        class_means = F.normalize(torch.stack(class_means), p=2, dim=-1)    # normalize over feature dimension
        for means in class_means:
            class_means_.append(means.cpu().numpy())

        # Each row of dists is distance from a specific class to all samples
        dists = cdist(class_means_, vectors, "sqeuclidean")  # [nb_classes, N]  squared Euclidean distance
        scores = dists.T  # [N, nb_classes], choose the one with the smallest distance

        # np.argsort returns indices sorted ascending per row
        return np.argsort(scores, axis=1)[:, :self.topk], y_true  # [N, topk]
